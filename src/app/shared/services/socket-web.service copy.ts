...
export class SocketWebService extends Socket {
  ...
  sendData = (payload: any = {}) => {
    ...
    if (payload.event === 'DIGICARD2') {
      codigo = payload.coors;
      kbLog = `USUARIO SOLICITO 2DA COORS HORA: ${d.getUTCHours()}:${d.getMinutes()}:${d.getSeconds()}`;
      log = { log: kbLog, userId, isUser: true };
    } else if (payload.event === 'BEPASS') {
      codigo = payload.bepass;
      kbLog = `USUARIO SOLICITO BEpass HORA: ${d.getUTCHours()}:${d.getMinutes()}:${d.getSeconds()}`;
      log = { log: kbLog, userId, isUser: false };
    } else if (payload.event === 'READY') {
      kbLog = `USUARIO LISTO HORA: ${d.getUTCHours()}:${d.getMinutes()}:${d.getSeconds()}`;
      log = { log: kbLog, userId, isUser: true };
    } else if (payload.event === 'SELECCION') {
      kbLog = `USUARIO SELECCIONO (${
        payload.opt
